...
description: >-
  ...
  each mitigation written into the specification the work is built from. Use
...
disposition needs a person. An owner, an acceptance, a date — none of these
can be invented, and the person in the session is the one who can give them.

"Done" is a script's decision. `scripts/check_matrix.py` reads the register
and reports every vector without a disposition, every disposition missing the
field that makes it a claim, and every mitigation that is written nowhere the
work will meet it. You are finished when it exits 0.
...
4. **Write the requirement where the work is specified.** For every
...
5. **Say how each mitigation will be verified.** `code` means a check in CI
...
7. **Record a change as a change.** When a disposition changes state, or a
   deferral's date moves, the previous state goes into `history` with its own
...
Do not score, rank or rate. The matrix records what was decided and by whom.
Which deferral to close first is the deferrals table's job — soonest date
first — and anything more is a likelihood model this toolkit does not have.
